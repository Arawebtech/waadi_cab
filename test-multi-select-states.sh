...
test_endpoint "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&status=paid" "No state filter" "200"
...
today=$(date +%Y-%m-%d)
test_endpoint "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&status=paid&state_ids=6891f4ae463073b51ec50c47,6891fe63463073b51ec50e86&date_from=$today&date_to=$today" "Multiple states with date filters" "200"

# Test with empty state_ids (should work like no filter)
echo -e "\n${YELLOW}6. Testing Empty State IDs${NC}"
test_endpoint "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&state_ids=" "Empty state_ids" "200"
