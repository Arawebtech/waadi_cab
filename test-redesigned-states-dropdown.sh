...
STATES_RESPONSE=$(curl -s "http://localhost:4001/api/v1/states/admin")
echo "States API Response:"
echo "$STATES_RESPONSE" | head -5

echo -e "\n${YELLOW}2. Testing No States Filter (Should Work)${NC}"
test_endpoint "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&status=paid" "No states filter" "200"

echo -e "\n${YELLOW}3. Testing Single State Filter (Backward Compatibility)${NC}"
test_endpoint "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&state_id=6891f4ae463073b51ec50c47" "Single state filter" "200"

echo -e "\n${YELLOW}4. Testing Multiple States Filter (New Implementation)${NC}"
test_endpoint "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&state_ids=6891f4ae463073b51ec50c47,6891fe63463073b51ec50e86" "Multiple states filter" "200"

echo -e "\n${YELLOW}5. Testing Empty States Array${NC}"
test_endpoint "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&state_ids=" "Empty states array" "200"

echo -e "\n${YELLOW}6. Testing States with Other Filters${NC}"
today=$(date +%Y-%m-%d)
test_endpoint "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&state_ids=6891f4ae463073b51ec50c47,6891fe63463073b51ec50e86&status=paid&date_from=$today&date_to=$today" "States with other filters" "200"

echo -e "\n${YELLOW}7. Testing Response Time${NC}"
RESPONSE_TIME=$(curl -s -w "%{time_total}" -o /dev/null "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&state_ids=6891f4ae463073b51ec50c47,6891fe63463073b51ec50e86")
