...
if curl -s -f  https://mdk7v2f6-4001.inc1.devtunnels.ms/health > /dev/null; then
...
# 1. Dashboard API
test_api " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/dashboard" "Dashboard Stats" 1000

# 2. Bookings API - Simple query
test_api " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/bookings?page=1&limit=10&status=paid" "Bookings API (Simple)" 500

# 3. Bookings API - Complex query with date range
test_api " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/bookings?page=7&limit=20&status=paid&date_from=2025-09-27&date_to=2025-09-28&date_on=createdAt&sort_by=createdAt&sort_order=desc" "Bookings API (Complex)" 800

# 4. Bookings API - Search query
test_api " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/bookings?page=1&limit=10&search=WC" "Bookings API (Search)" 600

# 5. Users API
test_api " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/users?page=1&limit=10" "Users API" 800
...
for page in 1 5 10 20; do
    test_api " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/bookings?page=$page&limit=20&status=paid" "Bookings API (Page $page)" 800
done
