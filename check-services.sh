...
if curl -s -f https://192.168.1.4:3000 > /dev/null; then
    echo "✅ Admin dashboard is running"
    echo "   Response time: $(curl -s -w "%{time_total}s" https://192.168.1.4:3000 -o /dev/null)"
else
...
echo "Dashboard: $(curl -s -f https://192.168.1.4:3000 > /dev/null && echo "✅ Running" || echo "❌ Not Running")"
