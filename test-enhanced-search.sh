...
test_search() {
    ...
    local response=$(curl -s " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/bookings?page=1&limit=10&search=$search_term")
    local http_code=$(curl -s -w "%{http_code}" -o /dev/null " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/bookings?page=1&limit=10&search=$search_term")
...
curl -s -w "Response time: %{time_total}s\nHTTP Code: %{http_code}\n" \
  " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/bookings?page=1&limit=5&search=UP16&status=paid&date_from=$today&date_to=$today" \
  | tail -2
