...
test_states_filter() {
    ...
    local response=$(curl -s "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&state_ids=$state_ids")
    local http_code=$(curl -s -w "%{http_code}" -o /dev/null "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&state_ids=$state_ids")
...
today=$(date +%Y-%m-%d)
response=$(curl -s "http://localhost:4001/api/v1/admin/bookings?page=1&limit=3&state_ids=689225a9b9d2b7cc874ad375&status=paid&date_from=$today&date_to=$today")
...
start_time=$(date +%s%N)
curl -s "http://localhost:4001/api/v1/admin/bookings?page=1&limit=5&state_ids=689225a9b9d2b7cc874ad375" > /dev/null
end_time=$(date +%s%N)
