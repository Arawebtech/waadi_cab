...
test_vehicle_type_creation() {
    ...
    # Make the API call
    local response=$(curl -s -X POST " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/vehicle-types" \
        -H "Content-Type: application/json" \
        -d "{\"name\": \"$vehicle_type_name\", \"state_id\": \"$STATE_ID\"}")
    
    local http_code=$(curl -s -w "%{http_code}" -o /dev/null -X POST " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/vehicle-types" \
        -H "Content-Type: application/json" \
        -d "{\"name\": \"$vehicle_type_name\", \"state_id\": \"$STATE_ID\"}")
...
vt_response=$(curl -s " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/vehicle-types?state_id=$STATE_ID")
...
if [ "$vt_id" != "null" ] && [ "$vt_id" != "" ]; then
    ...
    toggle_response=$(curl -s -X PATCH " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/vehicle-types/$vt_id/toggle")
    ...
    if [ "$toggle_success" = "true" ]; then
        ...
        new_list=$(curl -s " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/vehicle-types?state_id=$STATE_ID")
        ...
        recreate_response=$(curl -s -X POST " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/vehicle-types" \
            -H "Content-Type: application/json" \
