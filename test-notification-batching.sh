...
user_count_response=$(curl -s " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/admin/users?limit=1")
...
notification_response=$(curl -s -X POST " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/push/send-to-all" \
...
maintenance_response=$(curl -s -X POST " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/push/send-to-all" \
  -H "Content-Type: application/json" \
  -d '{
...
start_time=$(date +%s%N)
performance_response=$(curl -s -X POST " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/push/send-to-all" \
  -H "Content-Type: application/json" \
  -d '{
...
service_response=$(curl -s " https://mdk7v2f6-4001.inc1.devtunnels.ms/api/v1/push/test")
